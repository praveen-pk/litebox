...
fn optee_smc_handler(smc_args_addr: usize) -> OpteeSmcArgs {
    ...
    let smc_result = if smc_args.func_id() == Ok(OpteeSmcFunction::ReturnFromRpc) {
        ...
        read_optee_msg_args_from_regd_shm(registered_shm_ref, regd_shm_offset).and_then(
        ...
        handle_optee_smc_args(&mut smc_args)
    };
    ...
    match smc_result {
        ...
        OpteeSmcResult::ReturnFromRpc {
            ...
            match curr_stage {
                RpcStage::LoadTaSize => {
                    handle_return_from_load_ta_rpc(&mut smc_args, &msg_args, &mut rpc_args);
                }
                RpcStage::ShmAlloc => {
                    handle_return_from_shm_alloc_rpc(&mut smc_args, &msg_args, &mut rpc_args);
                }
                _ => {
                    smc_args.set_return_code(OpteeSmcReturnCode::EBadCmd);
                }
...
fn handle_return_from_load_ta_rpc(
    ...
    if !write_next_rpc(smc_args, msg_args, rpc_args, context_id) {
        discard_rpc_context(context_id);
    }
}
...
fn handle_return_from_shm_alloc_rpc(
    ...
    if tmem.buf_ptr == 0
        || tmem.shm_ref == 0
        || tmem.size < requested_size
        || checked_memref_size(tmem.size).is_err()
        || rpc_context_map()
    ...
    if register_rpc_shm(&tmem).is_err() {
        discard_rpc_context(context_id);
        smc_args.set_return_code(OpteeSmcReturnCode::EBadCmd);
        return;
    }
